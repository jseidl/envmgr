...
#from apivault.helpers.loaders import load_backend, load_encryption
from apivault.backends.local import Local
from apivault.encryptions.plain import Plain
from apivault.constants import APIVAULT_HOME, APIVAULT_CONFIG, APIVAULT_HOME_PERM
...
def load_config():
    ...
    home_path = APIVAULT_HOME

    if not home_path.exists():
        LOGGER.info('apivault folder is missing. Trying to create')
        home_path.mkdir(mode=APIVAULT_HOME_PERM)
        LOGGER.info('apivault folder created at %s', APIVAULT_HOME)

    config_path = Path(home_path, APIVAULT_CONFIG)
